...
const buildWorkerUrl = (path: string) => {
    ...
    return new URL(path, PUTER_WORKER_URL).toString();
};
...
export const getCurrentUser = async () => {
    if (!PUTER_WORKER_URL) {
        console.warn('Missing VITE_PUTER_WORKER_URL; skip auth check.');
        return null;
    }

    try {
        const response = await puter.workers.exec(buildWorkerUrl('/api/auth/whoami'), {
            method: 'GET',
        });
        ...
        const data = (await response.json()) as { user?: { username?: string; uuid?: string } | null };
